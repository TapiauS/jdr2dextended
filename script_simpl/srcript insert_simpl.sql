...
('Jajaja, Muskatnuss herr Muller !',(SELECT id_dialogue FROM dialogue WHERE contenu_dialogue='Ach so, vous vouloir grosse casquette à pointe traditionnelle de mein land! Moi avoir ça derrière gros comptoir, gut gut gut ! Vous vouloir lancer apfel sur ma grosse pointe ?')),
...
('Non, je suis là pour me battre avec des gros durs !',(SELECT id_dialogue FROM dialogue WHERE contenu_dialogue='Tu verras ou je vais te les mettres ses objets !')),
('Oui biensur, je suis un trè bon pilote !',(SELECT id_dialogue FROM dialogue WHERE contenu_dialogue='J''ai besoin de récupérer une potion à Nana Moule Curry au restaurant in&out pour retrouver ma vitesse !')),
('Non, je n''ai pas le permis…',(SELECT id_dialogue FROM dialogue WHERE contenu_dialogue='Je suis en manque de vitesse, pouvez-vous m''aider ?')),
...
SELECT interaction.id_interaction,nom_interaction,nom_objectif,objectif.id_personnage as target,objectif.id_objet as find,objet.id_objet as recompense,objectif.id_objectif,objectif.id_dialogue as talk,validation FROM joue_un_role 
    ...
    JOIN valide ON objectif.id_objectif=valide.id_objectif and valide.id_personnage=1 ORDER BY id_interactio,ordre DESC;